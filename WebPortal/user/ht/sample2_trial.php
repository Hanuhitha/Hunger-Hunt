...
<header>
  ...
  <p><input type="submit" class="btn btn-info" value="Home" name="home"></p> 
</form>
</header>
</div>
<?php
...
//mysqli_select_d ("hotelbooking",$connect) or die ("Could not find database");
$sql4="DELETE FROM temp_hotel";
  $query1=$db->query($sql);
    if($query1)
    {
    echo "<script>alert('Are you sure to go BACK');</script>";
  echo "<script>
...
<form action="">
<p class="font_sts"><input type="checkbox" name="range1" class="range1" value="high">High</p><br>
<p class="font_sts"><input type="checkbox" name="range2" class="range2" value="medium">Medium</p> <br>
<p class="font_sts"><input type="checkbox" name="range3" class="range3" value="low">Low</p><br>
...
$(document).on('change','.range2',function(){
   var url = "ajax_search.php";
   $.ajax({
     type: "POST",
     url: url,
...
$(document).on('change','.range3',function(){
   var url = "ajax_search.php";
   $.ajax({
     type: "POST",
     url: url,
